...
namespace Pathfinding {
	...
	public class RecastGraph : NavmeshBase, IUpdatableGraph {
		...
		void IUpdatableGraph.UpdateArea (GraphUpdateObject guo) {
			...
			AstarProfiler.StartProfile("BuildUpgrade Tiles");

			// BuildUpgrade the new tiles
			for (int x = affectedTiles.xmin; x <= affectedTiles.xmax; x++) {
				for (int z = affectedTiles.ymin; z <= affectedTiles.ymax; z++) {
			...
			AstarProfiler.EndProfile("BuildUpgrade Tiles");
		}
		...
		protected NavmeshTile BuildTileMesh (Voxelize vox, int x, int z, int threadIndex = 0) {
			AstarProfiler.StartProfile("BuildUpgrade Tile");
			...
			AstarProfiler.StartProfile("BuildUpgrade Nodes");
			...
			AstarProfiler.EndProfile("BuildUpgrade Nodes");

			AstarProfiler.EndProfile("BuildUpgrade Tile");
			return tile;
		}
